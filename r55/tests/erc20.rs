...
use r55::{
    ...
    test_utils::{add_balance_to_db, get_calldata, get_selector_from_sig, initialize_logger, ALICE, BOB, CAROL},
};
use revm::InMemoryDB;
...
fn test_erc20_deployment() {
    let ERC20Setup { mut db, token, owner } = erc20_setup(ALICE);
...
fn test_erc20_mint() {
    ...
    let mint_result = run_tx(&mut db, &token, calldata_mint, &owner)
        .expect("Error executing tx");
...
fn test_erc20_transfer() {
    ...
    let calldata_transfer = get_calldata(selector_transfer, (recipient, transfer_amount).abi_encode());
    
    let transfer_result = run_tx(&mut db, &token, calldata_transfer, &owner)
        .expect("Error executing tx");
...
fn test_erc20_approve_and_transfer_from() {
    ...
    let approve_result = run_tx(&mut db, &token, calldata_approve, &owner)
        .expect("Error executing tx");
    ...
    let transfer_from_result = run_tx(&mut db, &token, calldata_transfer_from, &spender)
        .expect("Error executing tx");
    assert!(transfer_from_result.status, "TransferFrom transaction failed");
...
fn test_erc20_transfer_insufficient_balance() {
    ...
    run_tx(&mut db, &token, calldata_mint, &owner)
...
fn test_erc20_transfer_from_insufficient_allowance() {
    ...
    run_tx(&mut db, &token, calldata_approve, &owner)
        .expect("Error executing approve tx");
...
fn test_erc20_mint_unauthorized() {
    let ERC20Setup { mut db, token, owner: _ } = erc20_setup(ALICE);
...
fn test_erc20_zero_address_checks() {
    ...
    let selector_transfer = get_selector_from_sig("transfer(address,uint256)");
    let calldata_transfer = get_calldata(selector_transfer, (zero_address, mint_amount).abi_encode());
