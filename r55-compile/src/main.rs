...
fn main() -> eyre::Result<()> {
    ...
    for entry in fs::read_dir(examples_dir)? {
        ...
        if path.is_dir() {
            ...
            // Compile initcode and save in a the file
            compile_runtime(path)?;
            let deploy_bytecode = compile_with_prefix(compile_deploy, path)?;
            let deploy_path = output_dir.join(format!("{}.bin", contract_name));
