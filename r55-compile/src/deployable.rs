...
pub fn generate_deployable(contract: &ContractWithDeps) -> eyre::Result<()> {
    ...
    content.push_str("//! This file provides Deployable implementations for contract dependencies\n");
    ...
    for dep in &contract.deps {
        ...
        content.push_str(&format!("    type Interface = {}<ReadOnly>;\n\n", interface_name));
    ...
    info!("Generated {:?} for contract: {}", output_path, contract.name.ident);
