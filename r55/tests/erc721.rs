...
fn test_erc721_deployment() {
    let ERC721Setup { mut db, token, owner } = erc721_setup(ALICE);
...
fn test_erc721_approve_and_transfer_from() {
    ...
    let approve_result = run_tx(&mut db, &token, calldata_approve, &token_owner)
        .expect("Error executing tx");
    ...
    let transfer_result = run_tx(&mut db, &token, calldata_transfer_from, &spender)
        .expect("Error executing tx");
...
fn test_erc721_set_approval_for_all() {
    ...
    let approval_result = run_tx(&mut db, &token, calldata_set_approval, &token_owner)
        .expect("Error executing tx");
    assert!(approval_result.status, "SetApprovalForAll transaction failed");
    
    // Verify approval status
    let selector_is_approved = get_selector_from_sig("is_approved_for_all(address,address)");
    let calldata_is_approved = get_calldata(selector_is_approved, (token_owner, operator).abi_encode());
...
fn test_erc721_mint_already_exists() {
    ...
    run_tx(&mut db, &token, calldata_mint.clone(), &owner)
        .expect("Error executing first mint tx");
...
fn test_erc721_balance_of_zero_address() {
    let ERC721Setup { mut db, token, owner: _ } = erc721_setup(ALICE);
    let zero_address = Address::ZERO;
